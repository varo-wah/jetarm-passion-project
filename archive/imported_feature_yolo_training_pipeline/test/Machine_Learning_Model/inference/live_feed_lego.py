...
Purpose:
- Open camera feed
- Use yolo_detector.py backend
- Display detected LEGO boxes
- Show class name, confidence, angle, pixel center, and robot coordinates
...
from yolo_detector import detect_objects, choose_target, load_model
...
def draw_detections(frame, detections, target):
    ...
    if target:
        cv2.putText(
            frame,
            f"TARGET: lego_brick robot=({target['robot_x']},{target['robot_y']}) angle={target['angle']}",
            (20, 30),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (0, 0, 255),
...
def main():
    ...
    while True:
        ...
        annotated_frame = draw_detections(annotated_frame, detections, target)
